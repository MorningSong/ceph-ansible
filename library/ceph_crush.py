...
def generate_cmd(cluster, subcommand, bucket, bucket_type, containerized=None):
    ...
    if containerized:
        for piece in reversed(containerized.split()):
            cmd.insert(0, piece)
    return cmd
...
def sort_osd_crush_location(location, module):
    '''
    Sort osd_crush_location dict
    '''
    len(location) < 2 and fatal("You must specify at least 2 buckets.", module)
    [item for item in location if item[0] == "host"] or fatal("You must specify a 'host' bucket.", module)
    try:
        crush_bucket_types = [
            "host",
            "chassis",
...
def create_and_move_buckets_list(cluster, location, containerized=None):
    ...
    for item in location:
        ...
        if previous_bucket:
            ...
            cmd_list.append(generate_cmd(cluster, "move", previous_bucket[1], bucket_type + "=" + bucket_name, containerized))
        previous_bucket = item
    return cmd_list
